...
function M.get_matrices(bufnr, root)
    local ok, query = pcall(vim.treesitter.query.parse, "python", matrix_query_string)
...
"->" @op_arrow
(binary_operator
    left: (identifier) @base
    operator: "**"
    right: (integer) @exp) @superscript
]]
...
function M.get_symbols(bufnr, root)
    local ok, query = pcall(vim.treesitter.query.parse, "python", symbol_query_string)
    if not ok or not query then return {} end
    ...
    for id, node in query:iter_captures(root, bufnr, 0, -1) do
        ...
        elseif capture_name == "superscript" then
            local exp_node = nil
            for child in node:iter_children() do
                if child:type() == "integer" then exp_node = child end
            end
            if exp_node then
                local exp_text = vim.treesitter.get_node_text(exp_node, bufnr)
                local sub_map = {
                    ["0"]="⁰", ["1"]="¹", ["2"]="²", ["3"]="³", ["4"]="⁴",
                    ["5"]="⁵", ["6"]="⁶", ["7"]="⁷", ["8"]="⁸", ["9"]="⁹"
                }
                if sub_map[exp_text] then
                    table.insert(symbols, {
                        start_row = range.start_row,
                        start_col = range.start_col,
                        end_row = range.end_row,
                        end_col = range.end_col,
                        replacement = vim.treesitter.get_node_text(node:child(0), bufnr) .. sub_map[exp_text]
                    })
                end
            end
        end
