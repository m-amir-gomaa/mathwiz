...
function M.get_matrices(bufnr, root)
    local ok, query = pcall(vim.treesitter.query.parse, "javascript", matrix_query_string)
...
function M.get_symbols(bufnr, root)
    local ok, query = pcall(vim.treesitter.query.parse, "javascript", symbol_query_string)
    if not ok or not query then return {} end
